...
def augment2(image,label):
    degree = 45
    dgr = np.random.uniform(-degree, degree)
    image = tf.image.convert_image_dtype(image, tf.float32)
    image = tfa.image.rotate(image, dgr * np.pi / 180, interpolation='BILINEAR')
    return image,label
...
def main():
    ...
    model.compile(
        optimizer=tf.optimizers.SGD(lr=0.0000012, momentum=0.9),
        loss=tf.keras.losses.categorical_crossentropy,
        metrics=[tf.keras.metrics.categorical_accuracy],
    )

    log_dir='{}/lab4-11/ilcd-{}'.format(LOG_DIR, time.time())
    model.fit(
        train_dataset,
        epochs=75,
